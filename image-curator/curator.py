...
class ImageCurator:
    ...
    def evaluate_single(
        self, word: str, translation: str, image_url: str, context: str = ""
    ) -> CurationResult:
        """
        ...
        try:
            ...
            logger.info(
                f"'{word}': {status} (avg: {score.average_score:.1f}, relevance: {score.relevance})"
            )
            return result
    ...
    async def evaluate_batch(
        self, items: List[Dict], delay_between: float = 1.0
    ) -> List[CurationResult]:
        """
        ...
        logger.info(
            f"Batch complete: {approved} approved, {rejected} rejected, {errors} errors"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Image Curator for Vocabulary Learning"
    )
    ...
    parser.add_argument(
        "--threshold", type=float, default=7.0, help="Approval threshold (0-10)"
    )
    ...
    try:
        curator = ImageCurator(approval_threshold=args.threshold, model=args.model)
