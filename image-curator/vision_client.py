...
class VisionClient:
    ...
    def __init__(self, model: Optional[str] = None, host: str = 'http://localhost:11434'):
        ...
        if not self.model:
            logger.warning("No vision model available. Pull one with: ollama pull llama3.2-vision")
            logger.warning("Vision evaluation will not work until a model is installed.")
        else:
    ...
    def _detect_best_model(self) -> Optional[str]:
        ...
        try:
            ...
            logger.warning("No vision model found. Install one with: ollama pull llama3.2-vision")
            return None
...
Be strict: only recommend (true) if total score >= 28/40 AND relevance >= 7."""
        ...
        try:
            ...
            return ImageScore(
                relevance=0, clarity=0, appropriateness=0, quality=0,
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            'install_command': 'ollama pull llama3.2-vision' if not self.model else None
        }
