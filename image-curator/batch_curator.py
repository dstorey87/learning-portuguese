...
class BatchProgress:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            'percent_complete': round(self.processed / max(self.total_words, 1) * 100, 1),
...
class BatchCurator:
    ...
    async def process_word(self, item: Dict) -> bool:
        ...
        try:
            ...
            results = await self.orchestrator.search_and_evaluate(
                word=word,
                query=search_query,
                count=self.config.candidates_per_word
            )
            ...
            if not best_result:
                logger.warning(f"No image for '{word}' met minimum score {self.config.min_score}")
            ...
            image_record = await self._save_candidate(item, best_result, status='selected')
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(result.url) as resp:
                    if resp.status == 200:
                        ...
                        self.library.update_image(record.id, {
    ...
    async def run(self, vocabulary: Optional[List[Dict]] = None) -> Dict:
        ...
        logger.info(f"Batch complete: {self.progress.successful} successful, "
...
async def main():
    ...
    parser.add_argument('--candidates', type=int, default=3,
                       help='Candidates per word')
    parser.add_argument('--min-score', type=int, default=28,
                       help='Minimum score to accept (out of 40)')
    parser.add_argument('--gpu-throttle', type=int, default=75,
                       help='GPU throttle percentage')
    ...
    parser.add_argument('--no-download', action='store_true',
                       help='Do not download images locally')
