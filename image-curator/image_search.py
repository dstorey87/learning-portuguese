...
class ImageCache:
    ...
    def get(
        self, query: str, source: str = "all", max_age_hours: int = 24
    ) -> Optional[List[Dict]]:
...
class ImageSearchOrchestrator:
    ...
    def __init__(
        ...
        self.vision_client = (
            vision_client or create_vision_client() if enable_vision else None
        )
    ...
    async def search_and_evaluate(
                ...
                for image in images:
                    try:
                        score = await self._evaluate_image(
                            image.url, portuguese_word, english_translation
                        )
        ...
        # Cache results
        cache_data = [{"result": r.to_dict(), "score": s} for r, s in scored_results]
        self.cache.set(cache_key, cache_data)

        return scored_results[:return_count]

    async def _evaluate_image(
        self, image_url: str, portuguese_word: str, english_translation: str
    ) -> float:
        ...
        for i in range(0, len(words), batch_size):
            ...
            for word, result in zip(batch, batch_results):
                ...
                if isinstance(result, Exception):
                    ...
                    results[pt_word] = [
                        {**img.to_dict(), "score": score} for img, score in result
                    ]
                else:
                    results[pt_word] = []
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "vision_client": (
                self.vision_client.get_status() if self.vision_client else None
            ),
...
async def search_for_word(
    portuguese_word: str, english_translation: str, category: str = ""
) -> List[Dict]:
    """
