...
class CuratorWebSocketServer:
    ...
    async def handle_client(self, websocket: websockets.WebSocketServerProtocol, path: str):
        ...
        # Send current status
        await self._send(websocket, MessageType.STATUS, {
    ...
    async def _handle_start(self, config_data: Dict):
        ...
        if not await self.curator.initialize():
            await self._broadcast(MessageType.ERROR, {"error": "Failed to initialize curator"})
            return
    ...
    async def _handle_manual_select(self, data: Dict):
        ...
        if image_id and self.curator:
            ...
            await self._broadcast(MessageType.LOG, {
                "message": f"Manually selected image {image_id}"
            })
    ...
    async def _handle_manual_reject(self, data: Dict):
        ...
        if image_id and self.curator:
            ...
            await self._broadcast(MessageType.LOG, {
                "message": f"Manually rejected image {image_id}"
            })
    ...
    async def _run_curator(self):
        ...
        try:
            ...
            await self._broadcast(MessageType.LOG, {
                "message": f"Batch complete: {results}"
            })
